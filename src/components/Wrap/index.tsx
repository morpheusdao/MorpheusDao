...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
    ...
    const wOracleBalance = useSelector<IReduxState, string>(state => {
        return state.account.balances && state.account.balances.wOracle;
    });
    ...
    const memoAllowance = useSelector<IReduxState, number>(state => {
        return state.account.wrapping && state.account.wrapping.oracle;
    });
    ...
    const setMax = () => {
        if (isWrap) {
            setValue(oracleBalance);
        } else {
            setValue(wOracleBalance);
        }
    };
    ...
    const trimmedMemoBalance = trim(Number(oracleBalance), 6);
    const trimmedWmemoBalance = trim(Number(wOracleBalance), 6);

    const getBalance = () => (isWrap ? `${trimmedMemoBalance} ORACLE` : `${trimmedWmemoBalance} wORACLE`);
            ...
            <Paper className="ohm-card ohm-popover wrap-token-poper">
                <div className="cross-wrap wrap-cros-wrap">
                    ...
                    <div className="wrap-price" onClick={() => setIsWrapPrice(!isWrapPrice)}>
                        <p>
                            1 {isWrapPrice ? "ORACLE" : "wORACLE"} = {`${trim(wrapPrice, 4)} ${isWrapPrice ? "wORACLE" : "ORACLE"}`}
                        </p>
                    </div>
                </div>
