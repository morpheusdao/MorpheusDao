...
import { StakingHelperContract, CypherTokenContract, OracleTokenContract, StakingContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("stake/changeApproval", async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const timeContract = new ethers.Contract(addresses.CYPHER_ADDRESS, CypherTokenContract, signer);
    const memoContract = new ethers.Contract(addresses.ORACLE_ADDRESS, OracleTokenContract, signer);
    ...
    try {
        ...
        const text = "Approve " + (token === "cypher" ? "Staking" : "Unstaking");
        const pendingTxnType = token === "oracle" ? "approve_staking" : "approve_unstaking";
