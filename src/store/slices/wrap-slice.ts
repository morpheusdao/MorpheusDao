...
import { wOracleTokenContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const memoContract = new ethers.Contract(addresses.ORACLE_ADDRESS, wOracleTokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await memoContract.approve(addresses.WORACLE_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const wmemoAllowance = await memoContract.allowance(address, addresses.WORACLE_ADDRESS);
...
export const changeWrap = createAsyncThunk("wrapping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
    ...
    const wmemoContract = new ethers.Contract(addresses.WORACLE_ADDRESS, wOracleTokenContract, signer);
...
const calcWrapValue = async ({ isWrap, value, provider, networkID }: IWrapDetails): Promise<number> => {
    ...
    if (isWrap) {
        const wmemoValue = await wmemoContract.ORACLETowORACLE(amountInWei);
        wrapValue = wmemoValue / Math.pow(10, 18);
    } else {
        const memoValue = await wmemoContract.wORACLEToORACLE(amountInWei);
